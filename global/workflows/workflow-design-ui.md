...
version: 1
status: active
intent: Execute design ui with explicit authority, state, outputs, and evidence.
use_when: [the task matches design ui]
...
resume_contract: task-scoped .agents/workflows/design-ui.json using the workflows directory contract
next_workflows: [none]
profiles: [general]
---
...
| Research + Story (skill-storytelling) | **.agents/contexts/research-brief.md** + **.agents/contexts/story.md** — master narrative document driving all design | After PRODUCT.md exists, before any visual system is defined |
...
| Visual Brainstorm (incl. Phase 3C) | **.agents/contexts/motion-direction.md** — scroll motion direction | After DESIGN.md exists; Phase 3C creates motion-direction.md as single authority |

PRODUCT.md and DESIGN.md live at the project root alongside Anti-Gravity's `.agents/contexts/` files. Anti-Gravity contexts handle engineering and product truth; Impeccable contexts handle design truth.
...
| `/ui-craft` | **General UI build loop** — confirms flow and states, builds production code, then performs browser and accessibility verification | The default for product, SaaS, dashboard, and application UI |
...
**UI Craft is the general default.** Run `/ui-craft` for product and application interfaces. Spatial-profile work runs `/impeccable-craft` only after its spatial concept contract exists.
...
| `/ui-animate` | Purposeful product motion with reduced-motion and performance gates | Feedback, continuity, or hierarchy needs motion |
...
Build (via `/ui-animate`). The motion direction is captured in
`.agents/contexts/motion-direction.md` and consumed by `skill-cinematic-motion`.
...
Phase 3A: Design Identity (Impeccable takes over)
    ...
    │   └── Write .agents/contexts/research-brief.md
    ...
    │   ├── User picks one or blends elements
    │   └── Write .agents/contexts/story.md
    ...
    │   │   ├── Map patterns to scroll sections
    │   │   ├── Generate image briefs for Figma
    │   │   └── Write .agents/contexts/motion-direction.md
...
- motion-direction.md is consumed by `/ui-animate` and `/ui-craft` during a general build; spatial builds use the Impeccable counterparts
...
Step 7c — Client Layer (Impeccable takes over)
    ├── /ui-craft (general loop: flow → states → build → verify)
    │   OR
...
UI exists → identify the issue → pick the right workflow:
    ...
    Static?   → /ui-animate (reads motion-direction.md if it exists)
    Generic?  → /impeccable-delight
    Broken?   → /impeccable-audit → fix → /impeccable-polish
```

**Note:** If motion-direction.md was created during inception, `/ui-animate`
...
`/ui-animate` uses the motion contract in `skills/ui-ux/reference/motion-design.md`
...
The sections below handle UX tasks that do not require implementation—user flow mapping, state coverage, information architecture, accessibility audits, and interaction design. For building the interface, use `/ui-craft` above.
