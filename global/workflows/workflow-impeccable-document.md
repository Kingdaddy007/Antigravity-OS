...
version: 1
status: active
intent: Execute impeccable document with explicit authority, state, outputs, and evidence.
use_when: [the task matches impeccable document]
...
resume_contract: task-scoped .agents/workflows/impeccable-document.json using the workflows directory contract
next_workflows: [none]
profiles: [general]
---
...
If a `DESIGN.md` already exists, **do not silently overwrite it**. Show the user the existing file and ask the user a concise blocking question whether to refresh, overwrite, or merge.
...
5. **Refresh the session cache.** Run `node "$IMPECCABLE_SCRIPTS/load-context.mjs"` one final time so the newly-written DESIGN.md lands in conversation. Subsequent commands in this session will use the fresh version automatically without re-reading.
...
Before interviewing:
1. Check if `.agents/contexts/story.md` is present in the active workspace.
2. If `.agents/contexts/story.md` is present:
   ...
   - Inform the user: "I detected an existing `.agents/contexts/story.md` in the workspace. I will use its narrative parameters to seed your `DESIGN.md` automatically, skipping the manual questions. OK?"
3. If `.agents/contexts/story.md` is NOT present or the user overrides it:
...
### Step 2: Five questions (Skip if .agents/contexts/story.md was parsed in Step 1)
...
3. Run `node "$IMPECCABLE_SCRIPTS/load-context.mjs"` once so the seed lands in conversation for the rest of the session.
